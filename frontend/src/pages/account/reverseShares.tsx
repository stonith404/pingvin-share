...
const MyShares = () => {
    ...
    return (
        <>
            <Meta title={intl.formatMessage({id: "account.reverseShares.title"})}/>
            <Group position="apart" align="baseline" mb={20}>
                <Group align="center" spacing={3} mb={30}>
                    <Title order={3}><FormattedMessage id="account.reverseShares.title"/></Title>
                    <Tooltip
                        position="bottom"
                        multiline
                        width={220}
                        label={intl.formatMessage({id: "account.reverseShares.description"})}
            ...
            {reverseShares.length == 0 ? (
                <Center style={{height: "70vh"}}>
                    <Stack align="center" spacing={10}>
                        <Title order={3}><FormattedMessage id="account.reverseShares.title.empty"/></Title>
                        <Text><FormattedMessage id="account.reverseShares.description.empty"/></Text>
                    </Stack>
                </Center>
            ) : (
                <Box sx={{display: "block", overflowX: "auto"}}>
                    <Table>
                        <thead>
                        <tr>
                            <th><FormattedMessage id="account.reverseShares.table.shares" /></th>
                            <th><FormattedMessage id="account.reverseShares.table.remaining" /></th>
                            <th><FormattedMessage id="account.reverseShares.table.max-size" /></th>
                            <th><FormattedMessage id="account.reverseShares.table.expires" /></th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                                                ...
                                                <Accordion.Control p={0}>
                                                    <Text size="sm">
                                                        {reverseShare.shares.length == 1 ? (
                                                            `1 ${intl.formatMessage({id: "account.reverseShares.table.count.singular"})}`
                                                        ) : (
                                                            `${reverseShare.shares.length} ${intl.formatMessage({id: "account.reverseShares.table.count.plural"})}`
                                                        )}
                                                    </Text>
                                                </Accordion.Control>
                                                <Accordion.Panel>
                                            ...
                                            onClick={() => {
                                                modals.openConfirmModal({
                                                    ...
                                                    children: (
                                                        <Text size="sm">
                                                            <FormattedMessage
                                                                id="account.reverseShares.delete.confirm"/>
                                                        </Text>
                                                    ),
                                                    confirmProps: {
                                                        color: "red",
                                                    ...
                                                    labels: {
                                                        confirm: intl.formatMessage({id: "common.button.delete"}),
